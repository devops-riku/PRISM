...
One file per person under the workspace's own `_inbox/`, for the same reason
the roster lives there: a workspace is a thing you can copy, back up or delete
in one piece, and everything about it should travel with it.
...
from app import hub, members, workspaces
...
def _read(person: str) -> tuple:
    ...
    """
    with _lock:
        cached = _files.get(_cache_key(person))
    if cached is not None:
        return Mail.model_validate(cached), True

    path = _path(person)
    try:
        if path.is_file():
            mail = Mail.model_validate_json(path.read_text(encoding="utf-8"))
        else:
            mail = Mail(person=person)
    except (OSError, ValueError) as exc:
        logger.warning("Unreadable inbox at %s - leaving it alone: %s", path, exc)
...
def _save(mail: Mail) -> None:
    ...
    try:
        path = _path(mail.person)
        path.parent.mkdir(parents=True, exist_ok=True)
        temporary = path.with_suffix(".json.tmp")
        temporary.write_text(mail.model_dump_json(indent=2), encoding="utf-8")
        temporary.replace(path)
    except OSError as exc:  # pragma: no cover - a full disk must not lose work
        logger.warning("Could not write the inbox for %s: %s", mail.person, exc)
