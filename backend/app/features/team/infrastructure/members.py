...
def load() -> Roster:
    """This workspace's roster. A missing row is an unclaimed workspace."""
    found = database.get(workspaces.current(), RECORD_KIND, RECORD_ID)
    if found is None:
        return Roster()
    try:
        return Roster.model_validate(found.payload)
    except ValueError as exc:
        logger.warning("Unreadable roster in workspace %s: %s", workspaces.current(), exc)
        return Roster()
...
def find_invite(token: str) -> tuple[str, Invite] | None:
    ...
    one means looking for it, which is a scan of one small roster aggregate per
    workspace.
    """
