...
That shape is the team mail adapter's, deliberately, down to `configured()` being a
...
imports this module and the API imports that, so a top-level import would
...
#: `closed`, and `app/presentation/api/intakes.py`'s `close_intake` is an `async def` that calls
...
#: running `close()` off the loop, which is the route's to do (this codebase
...
def _spaces_put(intake_id: str, filename: str, data: bytes, content_type: str) -> None:
    ...
    try:
        ...
        # bucket and the endpoint, and the route layer turns an unexpected failure into
