...
def resolve(token: str) -> tuple[str, str] | None:
    ...
    with _lock:
        ...
        if found is None and not _built:
            ...
            found = _index.get(wanted)
        if found is None:
            # PostgreSQL may be shared by more than one process, so the local
            # cache can miss a token another worker just minted. The indexed
            # SQL lookup is authoritative and remains O(1) for unknown tokens.
            stored = database.find_by_lookup("intake", wanted)
            if stored is not None:
                found = (stored.scope, stored.record_id)
                _index[wanted] = found
    if found is None:
        return None
