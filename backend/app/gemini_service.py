...
from app.costing import gross_for_target
...
async def generate_estimate(
    ...
    caller. Arithmetic is *not* corrected here - `app.costing.recompute` owns it.

    `kind` is a discipline id from `app.kinds` and decides the shape of the
...
async def revise_estimate(
    ...
    Arithmetic is not settled here. `app.costing.recompute` corrects it, and
    `app.costing.snap_to_total` lands it on `target_total` exactly. The model's
