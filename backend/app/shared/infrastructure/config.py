...
`app.infrastructure.gemini_service`. That lets `/api/health` answer honestly instead of
...
APP_DIR: Path = Path(__file__).resolve().parents[2]      # backend/app
...
#:
#: 5174, because `vite.config.ts` sets `strictPort: true` on that port for both
#: `dev` and `preview`: the dev server either binds 5174 or refuses to start, so
#: there is no configuration in which the app answers elsewhere and a default
...
# Empty means the zero-configuration local database at
# `GENERATED_DIR/prism.db`. Docker and production set this to a
# `postgresql+psycopg://...` URL. It is deliberately kept as the raw setting:
# the SQLite path is resolved lazily by `database.url()` so test processes that
# point GENERATED_DIR at a scratch directory receive their own database too.
...
#: somewhere to put them. Four of these or none: the intake file adapter's
#: `configured()`
...
#: Space there is, so the intake file adapter's `endpoint()` derives that when this
...
#: Read as text and added to the brief; see
#: `app.shared.infrastructure.attachments`. Fewer and larger
